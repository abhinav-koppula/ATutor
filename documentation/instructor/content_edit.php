...
<h2>Adding/Editing Content</h2>
	...
	<dl>
		...
		<dt>Multimedia</dt>
                <dd><p>Type the <kbd>[media][/media]</kbd> tags, along with a URL to an external media file, or a relative URL for a media file in the course File Manager (e.g. movies/mymovie.flv), to embed multimedia into your content. Supported formats currently include: mpeg, mov, wmv, swf, mp3, wav, ogg, mid, flv, mp4 and YouTube or Vimeo hosted videos. The media tag can take two parameters to define the width and height of the player when it displays <kbd>[media|640|480]http://www.youtube.com/watch?v=bxcZ-dFffHA[/media]</kbd> or perhaps <kbd>[media|960|680]http://vimeo.com/1721811[/media]</kbd>. If the parameters are not defined, the player size will default to 425x350. The media tag can also take an optional parameter to define the captions file to be used when it displays <kbd>[media captions=caption_file.srt]media_source[/media]</kbd> or perhaps <kbd>[media|640|480|captions=captions_file.srt]media_source[/media]</kbd>.The media tag can also take an optional parameter to define the transcripts file to be used when it displays <kbd>[media transcripts=transcript_file.json]media_source[/media]</kbd> or perhaps <kbd>[media|640|480|transcripts=transcript_file.json]media_source[/media]</kbd>.
                        <br/> Multiple transcripts can be specified in different languages by using the following syntax <kbd>[media|640|480|transcripts=transcript_file1.json:en|transcript_file2.json:fr|transcript_file3.json:es]media_source[/media]</kbd>. The above mentioned example specifies transcripts in three languages English(en), French(fr) and Spanish(es) using the files transcript_file1.json, transcript_file2.json and transcript_file3.json respectively. One can also use the <a href="content_media_interface.php">Media Interface</a> to specify the transcripts and the corresponding languages. If no language is specified, English(en) is used by default.
                        <br/>While in TinyMCE click on the film slides icon in the advanced editor to insert the media tags.</p>
